...
import { Spinner, EmptyState, Input, Select, Button } from "../components/ui";
...
export default function Explore() {
  ...
  async function aiSearch() {
    ...
    setAiSearching(true);
    const result = await aiSmartSearch(search, PRODUCT_TYPES.map((t) => t.label));
    const matchedType = PRODUCT_TYPES.find((t) => t.label.toLowerCase() === result.type.toLowerCase())?.value || "";
    setSearch(result.keywords);
    setType(matchedType);
    updateUrl({ search: result.keywords, type: matchedType });
    ...
    <div className="animate-fade">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="flex gap-2">
          <Input
            placeholder="🔍 Search products, tags... or describe what you need"
            value={search}
            onChange={(e) => {
          ...
          />
          <Button variant="soft" onClick={aiSearch} disabled={aiSearching} title="AI smart search">
            {aiSearching ? "..." : "✨"}
          </Button>
        </div>
        <Select
